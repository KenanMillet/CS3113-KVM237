...
Renderable::Renderable(Renderer* render)
...
const float * Renderable::Renderpack::getVertexColors() const
{
	if(okay) return &(vertexColors.front());
	return nullptr;
}

size_t Renderable::Renderpack::getSize() const
{
	if(okay) return vertexCoords.size();
	return 0;
}

Renderer* Renderable::defaultRender = &::render;