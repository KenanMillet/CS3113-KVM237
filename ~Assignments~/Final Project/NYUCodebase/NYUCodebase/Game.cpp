...
Game::Game(const char* name, ShaderProgram** const program, Renderer& render, Ticker& tick, CollisionHandler& ch, EventHandler& eh, LevelHandler& ll, ViewHandler& vh)
	: EventDependent({ SDL_QUIT, SDL_WINDOWEVENT, SDL_KEYDOWN }), fin(false), is_setup(false), programHandle(program), render(render), tick(tick), handleCollisions(ch), handleEvents(eh), handleLevels(ll), handleViews(vh)
{
	setup(name);
}
...
void Game::eventFilter(SDL_Event e)
{
	switch(e.type)
	{
		...
		case SDL_WINDOWEVENT:
			...
			break;
		case SDL_KEYDOWN:
			if(e.key.keysym.scancode == SDL_SCANCODE_ESCAPE) exit();
			break;
	}
}
...
Game& Game::setup(const char* name)
{
	...
	defaultView = new Camera(viewMatrix, projectionMatrix, &handleViews);
...
Game& Game::run()
{
	while(!fin)
	{
		if(handleLevels.empty())
		{
			handleLevels.addToQueue<TitleScreen>();
			handleLevels.addToQueue<MainGame>();
			handleLevels.addToQueue<GameOver>();
		}
		handleLevels();
		processEvents();
		updateObjects();
...
Renderer& Game::defaultRender = ::render;
Ticker& Game::defaultTick = ::tick;
